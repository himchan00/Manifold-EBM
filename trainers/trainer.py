...
class BaseTrainer:
    ...
    def train(self, model, d_dataloaders, logger=None, logdir=""):
        ...
        z_shape = (bs, z_dim)
        # model.encoder.load_state_dict(torch.load("pretrained/encoder_nae_ho_1.pth"))
        ...
        torch.save(model.encoder.state_dict(), "pretrained/encoder_z_dim_15.pth")
        torch.save(model.decoder.state_dict(), "pretrained/decoder_z_dim_15.pth")
        for i_epoch in range(1, cfg['n_epoch_ebm'] + 1):
            for x, _ in train_loader:
                ...
                if i_iter % cfg.visualize_interval == 0:
                    ...
                    logger.add_val(i_iter, d_val)
                i_iter += 1
        torch.save(model.ebm.net.conv_e.state_dict(), "pretrained/ebm_z_dim_15.pth")
        # model.ebm.net.conv_e.load_state_dict(torch.load("pretrained/ebm_conv_layer.pth"))
        ...
        if not cfg['fix_decoder']:
            ...
            if model.train_sigma:
                optimizer = optim.Adam([# 'params': model.encoder.parameters(), 'lr': cfg.optimizer['lr_encoder']},
                                        {'params': model.decoder.parameters(), 'lr':cfg.optimizer['lr_decoder']},
                                        {'params': model.sigma.conv_s.parameters(), 'lr': cfg.optimizer['lr_sigma']},
                                        {'params': model.ebm.net.conv_e.parameters(), 'lr': cfg.optimizer['lr_energy']}
                                        ])
            else:
                optimizer = optim.Adam([#{'params': model.encoder.parameters(), 'lr': cfg.optimizer['lr_encoder']},
                                        {'params': model.decoder.parameters(), 'lr':cfg.optimizer['lr_decoder']},
            ...
            for param in model.ebm.net.conv_e.parameters():
                param.requires_grad = False
            self.optimizer_pre = optim.Adam([{'params': model.encoder.parameters(), 'lr': cfg.optimizer['lr_encoder']}])
            if model.train_sigma:
                optimizer = optim.Adam([# {'params': model.encoder.parameters(), 'lr': cfg.optimizer['lr_encoder']},
                                        {'params': model.sigma.conv_s.parameters(), 'lr': cfg.optimizer['lr_sigma']}
                                        # {'params': model.ebm.net.conv_e.parameters(), 'lr': cfg.training.optimizer['lr_energy']}
                                        ])
            else:
                # optimizer = optim.Adam([{'params': model.encoder.parameters(), 'lr': cfg.optimizer['lr_encoder']},
                #                         # {'params':model.ebm.net.conv_e.parameters(), 'lr': cfg.training.optimizer['lr_energy']}
