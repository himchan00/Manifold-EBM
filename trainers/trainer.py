...
class BaseTrainer:
    ...
    def train(self, model, d_dataloaders, logger=None, logdir=""):
        ...
        for i_epoch in range(1, cfg['n_epoch'] + 1):
            for x, _ in train_loader:
                ...
                z_given_x = sample_langevin_z_given_x(x.to(self.device), energy = model.ebm, 
                                                                sigma = model.sigma, decoder = model.decoder, 
                                                                encoder = model.encoder, stepsize = model.ebm.conditional_step_size, 
                                                                n_steps = model.ebm.conditional_sample_step, temperature = model.ebm.temperature, spherical=True)
    ...
    def predict_new(self, m, dl, device, flatten=False, pretrain = False):
        ...
        for x, _ in dl:
            pred = []
            for i in range(1):
                z_given_x = sample_langevin_z_given_x(x.to(device), energy = m.ebm, 
                                                    sigma = m.sigma, decoder = m.decoder, 
                                                    encoder = m.encoder, stepsize = m.ebm.conditional_step_size, 
                                                    n_steps = m.ebm.conditional_sample_step, temperature = m.ebm.temperature, spherical=True)
                with torch.no_grad():
                    if flatten:
                        x = x.view(len(x), -1)
