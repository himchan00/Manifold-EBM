...
def get_ae(**model_cfg):
    ...
    if arch == "vae":
        ...
        from models.modules import FC_for_decoder_and_sigma, FC_for_encoder_and_sigma, normalized_net, sigma_net_normalizer
        ...
        if model_cfg["energy_ae"]["sigma_train"] == "sigma":
            sigma_net = get_net(in_dim=x_dim, out_dim=1, **model_cfg["sigma"])
            sigma = sigma_net_normalizer(sigma_net, 0.03, 0.3)
        else:
            sigma = None
